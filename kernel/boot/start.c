...
void timerinit(void) {
  ...
  *mtimecmp = *mtime + interval;
  ...
  printf("timerinit done for hart %d\n", r_mhartid());
}
...
void start(void) {
  ...
  /* ================================================================
   * 配置 PMP（Physical Memory Protection，物理内存保护）
   *
   * RISC-V 规定：若无任何 PMP 条目与 S-Mode 的访问匹配，该访问默认被拒绝。
   * 即：在 mret 降权到 S-Mode 之前，若不配置 PMP，S-Mode 连第一条
   * 取指令都会触发 PMP 访问故障，系统立刻崩溃。
   *
   * 此处的配置含义：
   *   pmpaddr0 = 0x3fffffffffffffull
   *     → TOR 模式下覆盖物理地址范围 [0, pmpaddr0 * 4) ≈ 全部物理内存
   *   pmpcfg0  = 0xf = 0b00001111
   *     → bit0(R)=1 bit1(W)=1 bit2(X)=1 bit[4:3](A)=01(TOR)
   *     → 赋予 S-Mode 对全部物理内存的读/写/执行权限
   *
   * 这是最简单的"全通行证"配置，适用于无需细粒度内存隔离的教学内核。
   * 框架已提供，无需修改。
   * ================================================================ */
